...
class mysql{
	 ...
	 * @param string $charset 字符集/编码
	 * @return bool  连接成功或不成功 
	 */
	function connect($config){
		if(!($con = mysql_connect($config['host'],$config['user'],$config['password']))){//mysql_connect连接数据库函数
			$this->err(mysql_error());
		}
		if(!mysql_select_db($config['name'],$con)){//mysql_select_db选择库的函数
			$this->err(mysql_error());
		}
		mysql_query("set names ".$config['charset']);//使用mysql_query 设置编码  格式：mysql_query("set names utf8")
	}
	/*
	 * 执行sql语句
	 * @param string $sql
